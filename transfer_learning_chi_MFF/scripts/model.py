...
class ChiMLP(nn.Module):
    ...
    def __init__(self, input_dim: int, n_layers: int, hidden_dims: List[int], dropout_rate: float):
        """
        Initialize MLP.
        ...
        Args:
            ...
            hidden_dims: List of dimensions for each hidden layer (length must equal n_layers)
        ...
        if len(hidden_dims) != n_layers:
            raise ValueError(f"Length of hidden_dims ({len(hidden_dims)}) must equal n_layers ({n_layers})")
