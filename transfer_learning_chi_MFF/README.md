...
- **Features**: Five modes available (selected via hyperparameter optimization)
  - `fp_T`: Morgan fingerprints (binary, 1024 bits) + temperature
  - `desc_T`: RDKit descriptors (~150 features) + temperature
  - `fp_desc_T`: Morgan fingerprints + RDKit descriptors + temperature
  - `fpf_T`: Morgan fingerprints with frequency (variable dim, ~10-50 features) + temperature
  - `fpf_desc_T`: Morgan fingerprints with frequency + RDKit descriptors + temperature
...
- Search space includes:
  - Feature mode (fp_T, desc_T, fp_desc_T, fpf_T, fpf_desc_T)
...
**Output:**
- `Data/features_fp.pkl` - Morgan fingerprints (binary, 1024 bits)
- `Data/features_fpf.pkl` - Morgan fingerprints with frequency (variable dim)
- `Data/features_fpf_metadata.pkl` - FPF metadata (hash codes, filtering info)
...
**Note on Frequency Fingerprints (fpf):**
- The `fpf` features use Morgan fingerprints with frequency counts instead of binary values
- Dimensionality is controlled by the `number_of_zero` parameter in `config.yaml` (default: 10000)
- Features are filtered to keep only hash codes that appear frequently in the DFT dataset
- Lower `number_of_zero` values = more features retained (more selective filtering)
- No normalization is applied - raw frequency counts are used
...
- Data paths
- Feature settings:
  - `morgan_fp`: Binary fingerprint radius and number of bits
  - `morgan_fpf`: Frequency fingerprint radius and `number_of_zero` filtering parameter
